"""
Unit tests for expense_service.
Uses MagicMock to fake ExpenseRepo — no real database needed.
"""
...
def make_expense(**overrides) -> Expense:
    """Helper to build a valid Expense with sensible defaults, override as needed."""
    defaults = dict(
        id=1,
        cost=Decimal("25.00"),
...
def test_check_ownership_raises_authorization_error_when_not_owner():
    repo = MagicMock()
    repo.get_expense_owner_id.return_value = 42  # different user
...
def test_update_expense_raises_authorization_error_for_wrong_owner():
    repo = MagicMock()
    repo.get_expense_owner_id.return_value = 42  # not this user
